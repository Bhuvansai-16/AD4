...
dataset_location = r'/home/anil/Downloads/AD'
for i in range(classes):
    path = os.path.join(dataset_location, 'Train', str(i))
    try:
        images = os.listdir(path)
        for a in images:
            try:
                print("inside loop")
                image = Image.open(os.path.join(path, a))  # Adjusted path construction
...
pred = model.predict(X_test)  # Get raw predictions
